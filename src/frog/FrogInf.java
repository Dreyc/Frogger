...
public class FrogInf implements IFrog {
    ...
    private int fps = 6; //après plusieurs essais, 6 est le meilleur nombre pour un déplacement agréable
    ...
    public FrogInf(Handler handler) {
        this.handler = handler;
        position = new Case((handler.getWidth()/2) * handler.getPixelByCase(), handler.getRealHeight() - 3*handler.getPixelByCase());
    }

    /* Update and render methods */

    public void update(){
        int idx = handler.getHeight() - this.position.ord / handler.getPixelByCase() + correction - 2;
    ...
    private void renderScore(Graphics g){
        ...
        int total = handler.getRealWidth()/2 - (Assets.widths.get(14)/2 + (score.length+2)*Assets.widths.get(11)/2 - Assets.widths.get(11)/2);
        int y = 0;
        g.drawImage(Assets.inGameScore[0], total, 0, Assets.widths.get(14), Assets.buttonSize, null);
        for(int i : score){
            g.drawImage(Assets.numbers[i], total + Assets.widths.get(14) + (y+1)*Assets.widths.get(11), 0, Assets.buttonSize/2 +3, Assets.buttonSize, null);
            y++;
        }
    }
    ...
    private void move() {
        if (handler.getKeyManager().up && position.ord - 1 >= 0) {
            frame = 0;
            if(canMove(1)){
                correction++;
                env.goUp();
        ...
        if (handler.getKeyManager().down && correction > 0) {
            frame = 1;
            if(canMove(2)){
                correction--;
                env.goDown();
